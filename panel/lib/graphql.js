...
const fetcher = async query => {
  ...
  if (jsonAccessToken.data) {
    ...
    const resConfirm = await fetch(process.env.NEXT_PUBLIC_API, {
      headers: {
        'Content-type': 'application/json',
    ...
    const jsonConfim = await resConfirm.json()
    if (!jsonConfim.errors) {
      return jsonConfim
    }
  }
...
const uploader = async formData => {
  ...
  if (jsonAccessToken.data) {
    ...
    const res2 = fetch(process.env.NEXT_PUBLIC_API, {
      headers: {
        authorization: 'Bearer ' + newAccessToken
      },
      method: 'POST',
