...
const Index = () => {
  ...
  const [_, deleteCategory] = useMutation(DELETE_CATEGORY)
  const remove = id => async () => {
    const res = await deleteCategory({ id })
    if (res.data.panelDeleteCategory === false) {
      alert('Desculpe, categoria pode estar em uso por um produto.')
      return
    }
  ...
  return (
    <Layout>
      ...
      <Button.Link href='/categories/create'>Nova Categoria</Button.Link>
        ...
        <div className='-my-2 py-2 overflow-x-auto sm:-mx-6 sm:px-6 lg:-mx-8 lg:px-8'>
          {data &&
            ...
            data.getAllCategories.length === 0 && (
              <Alert>Nenhuma categoria cadastrada.</Alert>
            )}
